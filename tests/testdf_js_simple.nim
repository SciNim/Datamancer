import datamancer, sequtils, math, strutils, streams, sugar, sets, tables
import algorithm
import seqmath
from os import removeFile

template suite(name, body: untyped): untyped =
  block:
    body
template test(name, body: untyped): untyped =
  block:
    body
template check(arg: untyped): untyped = doAssert arg
...
suite "DataTable tests":
  ...
  #  let summary = mpg.summarize(f{float: "mean_cyl" << mean(col("cyl"))},
  #                              f{float: "mean_hwy" << mean(col("hwy"))})
  #  echo "done "
  #  echo summary
  #  check almostEqual(summary["mean_cyl"][0, float], 5.88889)
  #  check almostEqual(summary["mean_hwy"][0, float], 23.4402)
  #  echo "almost queal"
  #  var sum_grouped = mpggroup.summarize(f{float: "mean_displ" << mean(col("displ"))},
  #                                       f{float: "mean_hwy" << mean(col("hwy"))})
  #  echo "summarized"
  #  echo sum_grouped
  ...
  #
  #  let cylFiltered = mpg.filter(f{idx("cyl") == 4})
  #  check cylFiltered.len == 81
  #  let cylDrvFiltered = cylFiltered.filter(f{idx("drv") == "4"})
  ...
  test "Filter - two comparisons using `and`":
    let x = toSeq(0 .. 100)
    let df = toDf(x)
    let dfFilter = df.filter(f{idx("x") >= 50 and
                               idx("x") <= 75})
  ...
  test "Filter - comparisons using function":
    let x = toSeq(0 .. 100)
    let df = toDf(x)
    let dfFilter = df.filter(f{float: idx("x") >= max(col("x")) * 0.5})
    check dfFilter["x"].toTensor(int) == toTensor toSeq(50 .. 100)

  test "Filter - data types":
    let x = toSeq(0 .. 100)
    let df = toDf(x)
    let dfFiltered = df.filter(f{float: idx("x") >= max(col("x")) * 0.5})
  ...
  test "Transmute - float arithmetic":
    ...
    let df = toDf(x, y)
    check df.len == 100
    let dfTrans = df.transmute(f{"x"}, f{"y2" ~ idx("y") * idx("y")})
  ...
  test "Gather - 2 columns":
    ...
    let dfY2FromLong = dfLong.filter(f{idx("from") == "y2"})
  ...
  test "Gather - 3 columns":
    ...
    let dfY1FromLong = dfLong.filter(f{idx("from") == "y1"})
    let dfY2FromLong = dfLong.filter(f{idx("from") == "y2"})
    let dfY3FromLong = dfLong.filter(f{idx("from") == "y3"})
